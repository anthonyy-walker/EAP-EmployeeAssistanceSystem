...
module.exports = {
    ...
    execute: async function (client, message, args, reaction, user, ) {
        ...
        const role = guild.roles.cache.find(role => role.name === 'Pro')
        let guild1 = client.guilds.cache.get('507924040636432388');
        const comp = await client.guilds.cache.get('653825414887768075');
        const mayhemlogs = await comp.channels.cache.get('750213240989941802');

        let e1 = new Discord.MessageEmbed()
        .setTitle('React for Pro Role')
    ...
    client.on("messageReactionAdd", async (reaction, user) => {
        ...
        if (member1.roles.cache.some(role => role.name === 'Pro')) {
